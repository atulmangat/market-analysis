...
export function StrategyReportPanel({ report, loading, error, onClose }: { report: StrategyReport | null; loading: boolean; error?: string | null; onClose: () => void }) {
  ...
  const assetBadgeClass: Record<AssetClass, string> = {
    crypto:    'bg-violet-900/60 text-violet-300 border-violet-500/30',
    stock:     'bg-surface3 text-brand-400 border-brand-500/30',
    commodity: 'bg-amber-900/60 text-amber-300 border-amber-500/30',
  };

  return (
    <div className="fixed inset-0 z-50 flex">
      <div className="flex-1 bg-black/50 backdrop-blur-sm" onClick={onClose} />
      <div className="w-full max-w-3xl h-full bg-surface border-l border-borderLight flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-borderLight bg-surface2 shrink-0">
            ...
            <div className="flex items-center gap-3 min-w-0">
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-base font-bold font-mono text-textMain tracking-tight">{s.symbol}</span>
                  <span className={`text-[9px] font-semibold px-1.5 py-0.5 rounded border tracking-widest ${assetBadgeClass[assetClass]}`}>
                    {assetLabel[assetClass]}
                  </span>
                </div>
                <p className="text-[10px] text-textDim mt-0.5">
                  Strategy Report · {new Date(s.timestamp).toLocaleString()}
                </p>
              </div>
            </div>
          ) : (
          ...
          <button
            onClick={onClose}
            className="text-textDim hover:text-textMain text-xl leading-none ml-4 shrink-0"
          >
            ×
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            ...
            <div className="p-6 text-center space-y-2">
              <p className="text-sm font-semibold text-down-text">Failed to load report</p>
